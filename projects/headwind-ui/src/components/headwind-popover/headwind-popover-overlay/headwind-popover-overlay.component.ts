...
export type HeadwindPopoverOverlayDirection = 'vertical' | 'horizontal';
export type HeadwindPopoverOverlayPositionPriority = 'left' | 'right' | 'top' | 'bottom';
...
export class HeadwindPopoverOverlayComponent implements AfterViewInit {
  @Input() direction: HeadwindPopoverOverlayDirection = 'vertical';
  @Input() directionPriority: HeadwindPopoverOverlayPositionPriority = 'bottom';
  @Input() pinPriority: HeadwindPopoverOverlayPositionPriority = 'left';

  @Output() actualDirectionChange = new EventEmitter<HeadwindPopoverOverlayPositionPriority>();
  @Output() actualPinChange = new EventEmitter<HeadwindPopoverOverlayPositionPriority>();
  ...
  private _updateVerticalDirectionByPriority(host: HTMLElement, hostDomRect: DOMRect, buttonDomRect: DOMRect): void {
    if (
      ...
      this._headwindWindowService.isBottomSpaceAvailable(buttonDomRect.bottom, hostDomRect.height)
    ) {
      this._renderer.setStyle(host, 'top', buttonDomRect.bottom + 'px');
      this._emitActualDirectionBottom();
    } else if (
      this.directionPriority === 'top' &&
      this._headwindWindowService.isTopSpaceAvailable(buttonDomRect.top, hostDomRect.height)
    ) {
      this._renderer.setStyle(host, 'bottom', `calc(100% - ${buttonDomRect.top}px)`);
      this._emitActualDirectionTop();
    } else if (this.directionPriority === 'top') {
      this._renderer.setStyle(host, 'bottom', `calc(100% - ${buttonDomRect.top}px)`);
      this._emitActualDirectionTop();
    } else {
      this._renderer.setStyle(host, 'top', buttonDomRect.bottom + 'px');
  ...
  private _updateHorizontalDirectionByPriority(host: HTMLElement, hostDomRect: DOMRect, buttonDomRect: DOMRect): void {
    if (
      this.directionPriority === 'right' &&
      this._headwindWindowService.isRightSpaceAvailable(buttonDomRect.right, hostDomRect.width)
    ) {
      this._renderer.setStyle(host, 'left', buttonDomRect.right + 'px');
      this._emitActualDirectionRight();
    } else if (
      this.directionPriority === 'left' &&
      this._headwindWindowService.isLeftSpaceAvailable(buttonDomRect.left, hostDomRect.width)
    ) {
      this._renderer.setStyle(host, 'right', `calc(100% - ${buttonDomRect.left}px)`);
      this._emitActualDirectionLeft();
    } else if (this.directionPriority === 'left') {
      this._renderer.setStyle(host, 'right', `calc(100% - ${buttonDomRect.left}px)`);
      this._emitActualDirectionLeft();
    } else {
      this._renderer.setStyle(host, 'left', buttonDomRect.right + 'px');
  ...
  private _updateVerticalDirectionPin(host: HTMLElement, hostDomRect: DOMRect, buttonDomRect: DOMRect): void {
    if (
      ...
      this._headwindWindowService.isRightSpaceAvailable(buttonDomRect.left, hostDomRect.width)
    ) {
      this._renderer.setStyle(host, 'left', buttonDomRect.left + 'px');
      this._emitActualPinLeft();
    } else if (
      this.pinPriority === 'right' &&
      this._headwindWindowService.isLeftSpaceAvailable(buttonDomRect.right, hostDomRect.width)
    ) {
      this._renderer.setStyle(host, 'right', `calc(100% - ${buttonDomRect.right}px)`);
      this._emitActualPinRight();
    } else if (this.pinPriority === 'right') {
      this._renderer.setStyle(host, 'right', `calc(100% - ${buttonDomRect.right}px)`);
      this._emitActualPinRight();
    } else {
      this._renderer.setStyle(host, 'left', buttonDomRect.left + 'px');
  ...
  private _updateHorizontalDirectionPin(host: HTMLElement, hostDomRect: DOMRect, buttonDomRect: DOMRect): void {
    if (
      this.pinPriority === 'top' &&
      this._headwindWindowService.isBottomSpaceAvailable(buttonDomRect.top, hostDomRect.height)
    ) {
      this._renderer.setStyle(host, 'top', buttonDomRect.top + 'px');
      this._emitActualPinBottom();
    } else if (
      this.pinPriority === 'bottom' &&
      this._headwindWindowService.isTopSpaceAvailable(buttonDomRect.bottom, hostDomRect.height)
    ) {
      this._renderer.setStyle(host, 'bottom', `calc(100% - ${buttonDomRect.bottom}px)`);
      this._emitActualPinTop();
    } else if (this.pinPriority === 'bottom') {
      this._renderer.setStyle(host, 'bottom', `calc(100% - ${buttonDomRect.bottom}px)`);
      this._emitActualPinTop();
    } else {
      this._renderer.setStyle(host, 'top', buttonDomRect.top + 'px');
